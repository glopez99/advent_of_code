...
function splitIntoSegmentsA (line) {
    var lineSegment;
  
    var lineSubstring = parseInt(line.substr(1));

        if (line.charAt(0) === 'R') {
            newXA = startingXA + lineSubstring;
            lineSegment = {direction: 'H', same: startingYA, beginning: startingXA, ending: newXA};
            startingXA = newXA;
            ...
            newYA = startingYA + lineSubstring;
            lineSegment = {direction: 'V', same: startingXA, beginning: startingYA, ending: newYA};
            startingYA = newYA;
            ...
            throw "You did something wrong in splitting Wire A into segments.";
        }
        return lineSegment;
}; 

function splitIntoSegmentsB (line) {
    var lineSegment;
  
    var lineSubstring = parseInt(line.substr(1));

        if (line.charAt(0) === 'R') {
            newXB = startingXB + lineSubstring;
            lineSegment = {direction: 'H', same: startingYB, beginning: startingXB, ending: newXB};
            startingXB = newXB;
        } else if (line.charAt(0) === 'L') {
            newXB = startingXB - lineSubstring;
            lineSegment = {direction: 'H', same: startingYB, beginning: startingXB, ending: newXB};
            ...
            newYB = startingYB + lineSubstring;
            lineSegment = {direction: 'V', same: startingXB, beginning: startingYB, ending: newYB};
            startingYB = newYB;
        } else if (line.charAt(0) === 'D') {
            newYB = startingYB - lineSubstring;
            lineSegment = {direction: 'V', same: startingXB, beginning: startingYB, ending: newYB};
            ...
            throw "You did something wrong splitting wire B into segments.";
        }
        return lineSegment;
}; 

var wireASegments = wireA.map(splitIntoSegmentsA);
var wireBSegments = wireB.map(splitIntoSegmentsB);

//console.log(wireASegments);
//console.log(wireBSegments);


function findIntersections(lineA, lineB) {
  var intersections = []
  for (let i = 0;  i < lineA.length; i++) {
    for (let j = 0; j < lineB.length; j++) { 
      intersections = [
        ...intersections,
        ...calculateIntersection(lineA[i], lineB[j])
      ]
    }
  }
...
function calculateIntersection(segmentA, segmentB) {
  if (segmentA.direction === segmentB.direction) {
    return [];
  } else {
    return calculatePerpendicularIntersections(segmentA, segmentB)
  }
};

function calculatePerpendicularIntersections(segmentA, segmentB) {
  if (getMin(segmentB) <= segmentA.same
      && segmentA.same <= getMax(segmentB)
      && getMin(segmentA) <= segmentB.same
      && segmentB.same <= getMax(segmentA)
  ) {
    if (segmentA.direction === 'V'){
      return [{x: segmentA.same, y: segmentB.same}];
    } else {
      return [{x: segmentB.same, y: segmentA.same}];
    }
  } else {
    return [];
  }
...
function getMax(segment) {
  var max = [segment.beginning, segment.ending];
  return Math.max(...max);
};

function getMin(segment){
  var min = [segment.beginning, segment.ending];
  return Math.min(...min);
};

function distance(segment){
  var distance = segment.x + segment.y;
  return distance;
};

//{direction: 'V', same: number, beginning: number, ending: number};
function isOnSegment(point, segment) {
  if (segment.direction === 'V') {
    return point.y > getMin(segment) && point.y < getMax(segment) //y is between beginning and end
      && point.x === segment.same; //x is same
  } else {
    return point.x > getMin(segment) && point.x < getMax(segment) //y is between beginning and end
      && point.y === segment.same; //x is same
  }
  throw 'neither';
}

function lengthAlong(point, segment) {
  const intersect = (segment.direction == 'V') ? point.y : point.x;
  return Math.abs(intersect - segment.beginning);
}

function length(segment) {
  return Math.abs(segment.ending - segment.beginning);
}

function distanceAlongWire(point, segments) {
  let distance = 0;
  for (let i = 0; i < segments.length; i++) {
    const segment = segments[i];
    if (isOnSegment(point, segment)) {
      return distance + lengthAlong(point, segment);
    } else {
      distance = distance + length(segment);
    }
  }
  throw "Not on line!";
}

function wireDistance(point) {
  return distanceAlongWire(point, wireASegments) + distanceAlongWire(point, wireBSegments);
}


var intersections = findIntersections(wireASegments, wireBSegments)
